#PPSplitFed-CNN-2:784 → 1440 →500|split|50 → 10 
import sys
import torch
...
def FedAvg_secure(w,y):
    ...
    for k in w_avg.keys():
        ...
        if num  == 1:
            w_avg[k]=w_avg[k].reshape(10, 1, 5, 5)
        elif num == 3:
            w_avg[k]=w_avg[k].reshape(20, 10, 3, 3)
...
def train_server(fx,fx_ct,fx_ct_T, y, onehot_y, l_epoch_count, l_epoch, idx, len_batch):
    ...
    fc1_b=net_server0.fc1.bias.data 
    fc1_w=net_server0.fc1.weight.data 
    fc1_wn=fc1_w.numpy()
    fc1_bn=fc1_b.numpy()

    sk_w0 = request_key_ndarray(fc1_wn)
    z1= serverexecute_ndarray(sk_w0, fx_ct, fc1_wn)
    ...
    b1_ones=np.ones(60)
    db1= dz1.dot (b1_ones)
    dfx= dz1.T.dot(fc1_wn)
    dfx=torch.Tensor(dfx)

    #update model
    optimizer_server1.step() 
    w1_new =  fc1_wn - lr * dw1 
    b1_new =  fc1_bn - lr * db1 
    w1_new=torch.Tensor(w1_new)
    b1_new=torch.Tensor(b1_new)   
    model_dict={'fc1.weight': w1_new, 'fc1.bias': b1_new}
    ...
    # count1: to track the completion of the local batch associated with one client追踪一个客户端是否完成本地batch
    count1 += 1
    if count1 == len_batch:
    ...
    def train(self, net):
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                dfx = train_server(fx_array,ct_ff_lst, ct_bf_lst,labels, onehot_labels,iter, self.local_ep, self.idx, len_batch)                           
...
for iter in range(epochs):
    ...
    #print("------ SL process at Client-Side and Server-side------- ")
    ct = dict()
    ct['parties'] = enrolled_parties
    ct['ct_dict'] = dict()
